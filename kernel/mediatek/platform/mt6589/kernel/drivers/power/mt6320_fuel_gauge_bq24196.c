...
void fgauge_get_profile_id(void)
{
	...
	//Get battery profile id, GPIO259
	mt_set_gpio_mode(GPIO259, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO259, GPIO_DIR_IN);
	fg_battery_id = (int)mt_get_gpio_in(GPIO259);

	printk("LOOKUP_TABLE : [fgauge_get_profile_id] battery id GPIO = %d\n", fg_battery_id);

	if((fg_battery_id != 0) && (fg_battery_id != 1)) {
	    printk("LOOKUP_TABLE : [fgauge_get_profile_id]Cannot get a valid battery id (%d)!\n", fg_battery_id);
	} else {
	    printk("LOOKUP_TABLE : [fgauge_get_profile_id]Battery id (%d)\n", fg_battery_id);
	}
}

int fgauge_get_saddles(void)
{
    return sizeof(battery_profile_t2_LG) / sizeof(BATTERY_PROFILE_STRUC);
}

int fgauge_get_saddles_r_table(void)
{
    return sizeof(r_profile_t2_LG) / sizeof(R_PROFILE_STRUC);
}
#else
int fgauge_get_saddles(void)
{
...
BATTERY_PROFILE_STRUC_P fgauge_get_profile(kal_uint32 temperature)
{
    ...
    printk ("LOOKUP_TABLE : fgauge_get_profile: fg_battery_id = %d , temperature=%d\n", fg_battery_id, temperature);
    switch (temperature)
    {
        case TEMPERATURE_T0:
		if(fg_battery_id == 0)
			return &battery_profile_t0_SANYO[0];
		else if(fg_battery_id == 1)
			return &battery_profile_t0_LG[0];
		else
			return &battery_profile_t0_SANYO[0];
	break;
        case TEMPERATURE_T1:
		if(fg_battery_id == 0)
			return &battery_profile_t1_SANYO[0];
		else if(fg_battery_id == 1)
			return &battery_profile_t1_LG[0];
		else
			return &battery_profile_t1_SANYO[0];
	break;
        case TEMPERATURE_T2:
		if(fg_battery_id == 0)
			return &battery_profile_t2_SANYO[0];
		else if(fg_battery_id == 1)
			return &battery_profile_t2_LG[0];
		else
			return &battery_profile_t2_SANYO[0];
	break;
        case TEMPERATURE_T3:
		if(fg_battery_id == 0)
			return &battery_profile_t3_SANYO[0];
		else if(fg_battery_id == 1)
			return &battery_profile_t3_LG[0];
		else
			return &battery_profile_t3_SANYO[0];
	break;
	case TEMPERATURE_T:
...
R_PROFILE_STRUC_P fgauge_get_profile_r_table(kal_uint32 temperature)
{
    ...
    printk ("LOOKUP_TABLE : fgauge_get_profile_r_table : fg_battery_id = %d , temperature=%d\n", fg_battery_id, temperature);
    switch (temperature)
    {
        case TEMPERATURE_T0:
		if(fg_battery_id == 0)
			return &r_profile_t0_SANYO[0];
		else if(fg_battery_id == 1)
			return &r_profile_t0_LG[0];
		else
			return &r_profile_t0_SANYO[0];
	break;
        case TEMPERATURE_T1:
		if(fg_battery_id == 0)
			return &r_profile_t1_SANYO[0];
		else if(fg_battery_id == 1)
			return &r_profile_t1_LG[0];
		else
			return &r_profile_t1_SANYO[0];
	break;
        case TEMPERATURE_T2:
		if(fg_battery_id == 0)
			return &r_profile_t2_SANYO[0];
		else if(fg_battery_id == 1)
			return &r_profile_t2_LG[0];
		else
			return &r_profile_t2_SANYO[0];
	break;
        case TEMPERATURE_T3:
		if(fg_battery_id == 0)
			return &r_profile_t3_SANYO[0];
		else if(fg_battery_id == 1)
			return &r_profile_t3_LG[0];
		else
			return &r_profile_t3_SANYO[0];
	break;
        case TEMPERATURE_T:
...
kal_int32 fgauge_read_capacity(kal_int32 type)
{
    ...
    //xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[fgauge_read_capacity] %d\r\n", dvalue);
    ...
    if (Enable_FGADC_LOG == 1){
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[fgauge_read_capacity] %d,%d,%d,%d,%d,D1=%d,D0=%d\r\n", 
    ...
    if (Enable_FGADC_LOG >= 1){
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[fgauge_read_capacity] dvalue=%d, dvalue_new=%d, gFG_BATT_CAPACITY_init_high_current=%d, gFG_BATT_CAPACITY_aging=%d\r\n", 
    ...
    if(dvalue <= 1)
    {
        dvalue=1;
        if (Enable_FGADC_LOG == 1){
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[fgauge_read_capacity] dvalue<=1 and set dvalue=1 !!\r\n");
        }
    }
...
kal_int32 fgauge_get_Q_max(kal_int16 temperature)
{
    ...
    {
        case 0:
		q_max_neg_10 = Q_MAX_NEG_10_SANYO;
		q_max_pos_0 = Q_MAX_POS_0_SANYO;
		q_max_pos_25 = Q_MAX_POS_25_SANYO;
		q_max_pos_50 = Q_MAX_POS_50_SANYO;
	break;
	case 1:
		q_max_neg_10 = Q_MAX_NEG_10_LG;
		q_max_pos_0 = Q_MAX_POS_0_LG;
		q_max_pos_25 = Q_MAX_POS_25_LG;
		q_max_pos_50 = Q_MAX_POS_50_LG;
	break;
	default:
		q_max_neg_10 = Q_MAX_NEG_10_SANYO;
		q_max_pos_0 = Q_MAX_POS_0_SANYO;
...
kal_int32 fgauge_get_Q_max_high_current(kal_int16 temperature)
{
    ...
    {
        case 0:
		q_max_neg_10_h_current = Q_MAX_NEG_10_H_CURRENT_SANYO;
		q_max_pos_0_h_current = Q_MAX_POS_0_H_CURRENT_SANYO;
		q_max_pos_25_h_current = Q_MAX_POS_25_H_CURRENT_SANYO;
		q_max_pos_50_h_current = Q_MAX_POS_50_H_CURRENT_SANYO;
	break;
	case 1:
		q_max_neg_10_h_current = Q_MAX_NEG_10_H_CURRENT_LG;
		q_max_pos_0_h_current = Q_MAX_POS_0_H_CURRENT_LG;
		q_max_pos_25_h_current = Q_MAX_POS_25_H_CURRENT_LG;
		q_max_pos_50_h_current = Q_MAX_POS_50_H_CURRENT_LG;
	break;
	default:
		q_max_neg_10_h_current = Q_MAX_NEG_10_H_CURRENT_SANYO;
		q_max_pos_0_h_current = Q_MAX_POS_0_H_CURRENT_SANYO;
