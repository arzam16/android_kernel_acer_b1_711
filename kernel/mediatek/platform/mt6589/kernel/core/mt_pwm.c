...
//Compal Add start
int odm_suspend_flag;
int odm_suspend_flag_debug_count = 0;
EXPORT_SYMBOL(odm_suspend_flag);
//Compal Add end
...
static ssize_t suspend_state_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	odm_suspend_flag_debug_count++;
	printk("%s : odm_suspend_flag = %d , try count : %d \n", __func__, odm_suspend_flag, odm_suspend_flag_debug_count);
	return sprintf(buf, "%d\n", odm_suspend_flag);
}

static ssize_t suspend_state_init_show(struct device *dev,
                                struct device_attribute *attr, char *buf)
{
	odm_suspend_flag = 0;
	odm_suspend_flag_debug_count = 0;
        printk("%s : state initial, odm_suspend_flag = %d\n", __func__, odm_suspend_flag);
        return sprintf(buf, "%d\n", odm_suspend_flag);
}
				 ...
				 const char *buf, size_t count)
{
	int val;
	sscanf(buf, "%d", &val);
	odm_suspend_flag = val;
	printk("%s : odm_suspend_flag = %d\n", __func__, odm_suspend_flag);
	return count;
}

static DEVICE_ATTR(suspend, S_IRWXUGO,
		   suspend_show, suspend_store);
*/

static DEVICE_ATTR(suspend_state, S_IRUGO,
                   suspend_state_show, NULL);

static DEVICE_ATTR(suspend_state_init, S_IRUGO,
                   suspend_state_init_show, NULL);

static struct attribute *suspend_attrs[] = {
	&dev_attr_suspend_state.attr,
	&dev_attr_suspend_state_init.attr,
	NULL,
};
...
int mt_pwm_probe ( struct platform_device *pdev)
{
	...
	kobj = kobject_create_and_add("odm_debug_state", NULL);
	if (!kobj) {
		PWMDBG ("failed to create kobject error\n");
	}

	retfs = sysfs_create_group(kobj, &suspend_attr_group);
	//Compal Add end
