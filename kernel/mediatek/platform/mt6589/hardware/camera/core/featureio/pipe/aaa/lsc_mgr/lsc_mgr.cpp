...
fillTblInfoByLscScenarionCT(SHADING_TBL_SPEC &tbl_sepc, ELscScenario_T lsc_scenario, MUINT8 ct)
{
    ...
    tbl_sepc.ratio_idx   = m_rIspLscCfg[lsc_scenario].ratio.val;
...
importEEPromData(void)
{
...
#if DEBUG_ALIGN_FUNC
    {
        ...
        UINT32* align_out_coef      = new UINT32 [MAX_SHADING_TIL_COEFF_SIZE/sizeof(UINT32)];
        UINT32* trfm_out_coef       = new UINT32 [MAX_SHADING_TIL_COEFF_SIZE/sizeof(UINT32)];
        ...
        shading_align_golden(align_conf);
        ...
        for (int i = 0; i < sizeof(align_result)/sizeof(align_result[0]); i++)
        {
            ...
            {
                MY_LOG("[%s] idx %d, (result, out) = (0x%08x, 0x08x)", __FUNCTION__,
    ...
    else if (table_type & 0x02)   // mtk lsc
    {
        //        typedef struct {
        //            MUINT8      MtkLscType; //LSC Table type    "[0]sensor[1]MTK"   1
        //            MUINT8      PixId; //0,1,2,3: B,Gb,Gr,R
        ...
        eeprom.ctl2.bits.SDBLK_WIDTH    = ((mtk_lsc->CapIspReg[1] >> 16) & 0x000000FF);
        eeprom.ctl3.bits.SDBLK_HEIGHT   =  (mtk_lsc->CapIspReg[1]    & 0x000000FF);
        eeprom.lblock.bits.SDBLK_lWIDTH = ((mtk_lsc->CapIspReg[3] >> 16) & 0x000000FF);
        eeprom.lblock.bits.SDBLK_lHEIGHT=  (mtk_lsc->CapIspReg[3]        & 0x000000FF);
        ...
        golden.ctl2.bits.SDBLK_WIDTH        = ((m_rIspShadingLut.SensorGoldenCalTable.IspLSCReg[1] >> 16) & 0x000000FF);
        golden.ctl3.bits.SDBLK_HEIGHT       =  (m_rIspShadingLut.SensorGoldenCalTable.IspLSCReg[1]    & 0x000000FF);
        golden.lblock.bits.SDBLK_lWIDTH     = ((m_rIspShadingLut.SensorGoldenCalTable.IspLSCReg[3] >> 16) & 0x000000FF);
        golden.lblock.bits.SDBLK_lHEIGHT    =  (m_rIspShadingLut.SensorGoldenCalTable.IspLSCReg[3]        & 0x000000FF);
...
#else
        virAddr = stRawLscInfo[m_eLscScenario].virtAddr +
                getPerLutSize(m_eLscScenario) * m_u4CTIdx;
        MUINT32 Addr =
...
updateLscScenarioByIspProfile(EIspProfile_T profile)
{
    ...
    switch(profile)
    {
        ...
        case EIspProfile_ZsdPreview_NCC:
            m_eLscScenario = LSC_SCENARIO_03;
            m_eSensorOp = ACDK_SCENARIO_ID_CAMERA_ZSD;
            break;
...
static ACDK_SCENARIO_ID_ENUM SensorScenarioIspProfileMapping[] =
{
        ...
        ACDK_SCENARIO_ID_CAMERA_ZSD,            //EIspProfile_ZsdPreview_CC
        ACDK_SCENARIO_ID_CAMERA_ZSD,            //EIspProfile_ZsdPreview_NCC
...
#if ENABLE_TSF
        if (bSceneChange == MTRUE)   // nothing changed
        {
...
enableLsc(MBOOL const fgEnable)
{
    ...
    if (m_bBypass) {
        MY_LOG("[LscMgr:%s] Bypass LSC \n", __FUNCTION__);
        return MTRUE;
    } else
        MY_LOG("[LscMgr:%s]  --> %d, m_eLscScenario %d\n", __FUNCTION__,
                fgEnable, m_eLscScenario);
...
MVOID
LscMgr::
prepareTSFInputBuffer(EIspProfile_T profile)
{
    ACDK_SCENARIO_ID_ENUM sensor_scenario = getSensorScenarioByIspProfile(profile);
    ELscScenario_T lsc_scenario =  getLscScenarioBySensorScenario(sensor_scenario);
    ...
    if (!(EIspProfile_NormalPreview == profile || EIspProfile_ZsdPreview_CC == profile)) {
        MY_LOG("[LscMgr:%s] !Preview from input vir 0x%08x to bak vir 0x%08x\n", __FUNCTION__,
        ...
        memcpy((void *)m_TSFBuff[TSF_BUFIDX_BAK].virtAddr, (void *)m_TSFBuff[TSF_BUFIDX_INPUT].virtAddr,
                getPerLutSize(m_ePrevLscScenario)); // D65?

        MY_LOG("[LscMgr:%s] from table vir 0x%08x to input vir 0x%08x\n", __FUNCTION__,
                stRawLscInfo[lsc_scenario].virtAddr+getTSFD65Idx()*getPerLutSize(lsc_scenario),
                m_TSFBuff[TSF_BUFIDX_INPUT].virtAddr);
#if USING_BUILTIN_LSC
...
#else
        memcpy((void *)m_TSFBuff[TSF_BUFIDX_INPUT].virtAddr,
                (void *)(stRawLscInfo[lsc_scenario].virtAddr+getTSFD65Idx()*getPerLutSize(lsc_scenario)),
                getPerLutSize(lsc_scenario)); // D65?
#endif
    } else {
        ...
        memcpy((void *)m_TSFBuff[TSF_BUFIDX_INPUT].virtAddr, (void *)m_TSFBuff[TSF_BUFIDX_BAK].virtAddr,
                m_TSFBuff[TSF_BUFIDX_BAK].size); // D65?
    }
    ...
    while (lsc->mTSFState != LSCMGR_TSF_STATE_EXIT)
    {
        ...
        switch(lsc->mTSFState)
        {
            ...
            case LSCMGR_TSF_STATE_SCENECHANGE:  //////////////////////////////////////////////////
                ...
                lsc->prepareTSFInputBuffer(lsc->getIspProfile());
...
//                }
            ...
            case LSCMGR_TSF_STATE_GETNEWINPUT:  //////////////////////////////////////////////////
                ...
                {
#if 0   // transform pv to cap table directly
                    MY_LOG("[LscMgr:%s] Normal Capture, MTKTSF_FEATURE_GEN_CAP_TBL\n", __FUNCTION__);
                    ret = tsf->TsfFeatureCtrl(MTKTSF_FEATURE_GEN_CAP_TBL, &TSFUpdateInfo, &TSFOutput);
                    if (ret != S_TSF_OK) {
                        MY_ERR("MTKTSF_FEATURE_GEN_CAP_TBL error %x", ret);
...
//                                lsc->getPerLutSize((NSIspTuning::LscMgr::ELscScenario_T)lsc->getRegIdx()));
//                        lsc->SetTBAToISP();
//                        lsc->enableLsc(MTRUE); // for cct, this should not called
...
//                    }
                    ...
                    UINT8 TimeoutCnt = 0;
                    do {
                        MY_LOG("[LscMgr:%s] Capture Smooth TsfMain cnt %d\n", __FUNCTION__,
                                TimeoutCnt);
                        ret = tsf->TsfMain();
                        if (ret != S_TSF_OK) {
                        ...
                        TimeoutCnt++;
                    } while (TSFGetProc.TsfState != MTKTSF_STATE_READY && TimeoutCnt < 14);
                ...
                if (EIspProfile_NormalCapture == lsc->getIspProfile())
                {
                    ...
                    if (ret != S_TSF_OK) {
                        ...
                        if (TSFGetProc.TsfState != MTKTSF_STATE_READY) {
                            MY_LOG("[LscMgr:%s] smooth TsfMain\n", __FUNCTION__);
                            ret = tsf->TsfMain();
                            if (ret != S_TSF_OK) {
                                MY_ERR("TsfMain error %x", ret);
                                tsf->TsfReset();
