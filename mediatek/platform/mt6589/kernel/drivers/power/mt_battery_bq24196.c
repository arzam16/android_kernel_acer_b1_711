...
// Smart Charging
////////////////////////////////////////////////////////////////////////////////
#define TEMP_NUM sizeof(Batt_VoltToTemp_Table)/sizeof(Batt_VoltToTemp_Table[0])
...
typedef struct{
	int prelevel;
	int oldTemp;
	int division; // temperature: 22000/1000=22
	char *node;
}MODULE_INFO;

MODULE_INFO Module_Table[] = {
	{1, 0, 1000, "/sys/devices/virtual/thermal/thermal_zone3/temp"}, // thermal
	{1, 0, 1000, "/sys/devices/virtual/thermal/thermal_zone2/temp"}, // battery
	{1, 0, 1, "/sys/class/power_supply/battery/bq24196_temp"}, // charger(bq24196)
};
#endif
...
kal_bool upmu_is_chr_det_usb(void)
{
    kal_uint32 tmp32;
...
static int bq24196_get_temperature() {

	int data[4], ret = 0;
	int Channel = 3;
	int adcVol = 0;
	int i = 0, bq24196_temp = 0;

	mt_set_gpio_pull_enable(GPIO116, GPIO_PULL_DISABLE);
	mt_set_gpio_dir(GPIO116, GPIO_DIR_IN);

	ret = IMM_GetOneChannelValue(Channel, data, NULL);
	adcVol = (data[0]*1000) + (data[1]*10); //mV

	while (i<TEMP_NUM) {
		if (adcVol == Batt_VoltToTemp_Table[i].bq24196_voltage) {
			bq24196_temp = Batt_VoltToTemp_Table[i].bq24196_temp;
			return bq24196_temp;
		}
	i++;
	}
...
static enum power_supply_property mt6320_battery_props[] = {
    ...
    POWER_SUPPLY_PROP_bq24196_temp,
};
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ChargerVoltage:
	    ...
	    break;
#ifdef COMPAL_SMART_CHARGING_SUPPORT
	case POWER_SUPPLY_PROP_bq24196_temp:
	    val->intval = bq24196_get_temperature();
	    break;
#endif
	default:
	    ret = -EINVAL;
        ...
        if ( BMT_status.bat_exist )
        {
            ...
            else
            {
                ...
                if (g_bat_full_user_view)
                {
					...
					xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Battery_Recharging][CDEBUG] Keep UI as 100. bat_volt_check_point=%d, BMT_status.SOC=%d\r\n", bat_volt_check_point, BMT_status.SOC);
                }
                else
                {
					...
					else
					{
						if(bat_volt_check_point >= 100 )
						{
							...
							xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Battery][CDEBUG] Use gas gauge : gas gague get 100 first (%d)\r\n", bat_volt_check_point);
						}
						else
						{
							if(bat_volt_check_point == BMT_status.SOC) {
								...
								xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Battery][CDEBUG] Can sync due to bat_volt_check_point=%d, BMT_status.SOC=%d\r\n", bat_volt_check_point, BMT_status.SOC);
							} else if ((bat_volt_check_point < BMT_status.SOC) && (bat_volt_check_point < 99)) {

								xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Battery][CDEBUG] bat_volt_check_point=%d,BMT_status.SOC=%d,gFGsyncTimer=%d(on %d)\r\n", bat_volt_check_point, BMT_status.SOC, gFGsyncTimer, DEFAULT_SYNC_TIME_OUT);
								...
								if(gFGsyncTimer >= DEFAULT_SYNC_TIME_OUT) {
									...
									xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Battery][CDEBUG] Keep UI due to bat_volt_check_point=%d, BMT_status.SOC=%d\r\n",
									bat_volt_check_point, BMT_status.SOC);
							}
						}
						bat_data->BAT_CAPACITY = bat_volt_check_point;
						printk ("[CDEBUG] : bat_data->BAT_CAPACITY=%d, \n", bat_data->BAT_CAPACITY);
					}
                }
            }
        }
...
void bq24196_set_ac_current(void)
{
	...
	//bq24196_set_ichg(0x10); //charging current limit at 1536mA
    }
}
...
#else
    //bq24196_set_vreg(0x2C); //VREG 4.208V
    bq24196_set_vreg(0x32); //VREG 4.3V
#endif    
...
int BAT_CheckBatteryStatus_bq24196(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		if ((BMT_status.temperature >= 5) && (BMT_status.temperature <= 45)) {
			printk ("[BATTERY:bq24196] Battery Temperature Between 5 and 45, Start Charging\n");
...
void BAT_thread_bq24196(void)
{
    ...
    if( upmu_is_chr_det_usb() == KAL_TRUE )
    {
...
int get_hw_temperature(int module) {
...
// step1: Read sys node
	fd = my_open(Module_Table[module].node, O_RDONLY);

	if (fd < 0) {
		printk("SMART_CHARGER : [get_hw_temperature]: open file fail (module %d)\n", module);
		return -100;
	}
...
// step3: Assign value
	got_value = got_value/(Module_Table[module].division);

	return got_value;
}

int Thermal_TempToLevel(int currentT) {

	int diff = 0;
	char *string = "temp";

	// diff>0: up, diff<0: down
	diff = (currentT) - (Module_Table[THERMAL_MODULE].oldTemp);

	if (diff > 0)	{
		if (currentT < 70)
			Module_Table[THERMAL_MODULE].prelevel = 1;
		else if (currentT >= 70 && currentT < 77)
			Module_Table[THERMAL_MODULE].prelevel = 2;
		else if (currentT >= 77)
			Module_Table[THERMAL_MODULE].prelevel = 3;
		string = "up";
	} else if (diff < 0) {
		if (currentT <= 65)
			Module_Table[THERMAL_MODULE].prelevel = 1;
		else if (currentT > 65 && currentT <= 72)
			Module_Table[THERMAL_MODULE].prelevel = 2;
		else if (currentT > 72)
			Module_Table[THERMAL_MODULE].prelevel = 3;
		string = "down";
	...
	printk ("SMART_CHARGER : Thermal(temp): Now=%d, Last=%d, %s=%d\n",
							currentT, Module_Table[THERMAL_MODULE].oldTemp, string, diff);
	return Module_Table[THERMAL_MODULE].prelevel;
}

int Battery_TempToLevel(int currentT) {

	int diff = 0;
	char *string = "temp";

	// diff>0: up, diff<0: down
	diff = (currentT) - (Module_Table[BATTERY_MODULE].oldTemp);

	if (diff > 0) {
		if (currentT < 50)
			Module_Table[BATTERY_MODULE].prelevel = 1;
		else if (currentT >= 50)
			Module_Table[BATTERY_MODULE].prelevel = 2;
		string = "up";
	} else if (diff < 0) {
		if (currentT <= 45)
			Module_Table[BATTERY_MODULE].prelevel = 1;
        ...
        printk ("SMART_CHARGER : Battery(temp): Now=%d, Last=%d, %s=%d\n",
                                                        currentT, Module_Table[BATTERY_MODULE].oldTemp, string, diff);
	return Module_Table[BATTERY_MODULE].prelevel;
}

int Charger_TempToLevel(int currentT) {

	int diff = 0;
	char *string = "temp";

	// diff>0: up, diff<0: down
	diff = (currentT) - (Module_Table[CHARGER_MODULE].oldTemp);

	if (diff > 0)   {
		if (currentT < 70)
			Module_Table[CHARGER_MODULE].prelevel = 1;
		else if (currentT >= 70 && currentT < 77)
			Module_Table[CHARGER_MODULE].prelevel = 2;
		else if (currentT >= 77)
			Module_Table[CHARGER_MODULE].prelevel = 3;
			string = "up";
	} else if (diff < 0) {
		if (currentT <= 65)
			Module_Table[CHARGER_MODULE].prelevel = 1;
	...
	printk ("SMART_CHARGER : Charger(temp): Now=%d, Last=%d, %s=%d\n",
							currentT, Module_Table[CHARGER_MODULE].oldTemp, string, diff);
	return Module_Table[CHARGER_MODULE].prelevel;
}

int LevelToIbatt(int *level) {

	int ibatt[MODULE_SUM] = {0};
	int minIbatt = 0;
	int i = 0;

// Step1: level => Ibattery
	ibatt[THERMAL_MODULE] = thermal_LevelToIbatt_Table[level[THERMAL_MODULE]].SM_ibatt;
	ibatt[BATTERY_MODULE] = battery_LevelToIbatt_Table[level[BATTERY_MODULE]].SM_ibatt;
	ibatt[CHARGER_MODULE] = charger_LevelToIbatt_Table[level[CHARGER_MODULE]].SM_ibatt;
	printk ("SMART_CHARGER : Thermal: level=%d, ibatt=%d\n", level[THERMAL_MODULE], ibatt[THERMAL_MODULE]);
	printk ("SMART_CHARGER : Battery: level=%d, ibatt=%d\n", level[BATTERY_MODULE], ibatt[BATTERY_MODULE]);
	printk ("SMART_CHARGER : Charger: level=%d, ibatt=%d\n", level[CHARGER_MODULE], ibatt[CHARGER_MODULE]);

// Step2: Select min Ibattery
	minIbatt = ibatt[0];
	for (i=1; i<MODULE_SUM; i++) {
		if (ibatt[i] < minIbatt) {
			minIbatt = ibatt[i];
		}
	}
...
void Smart_Charging(void) {

	int currentT[MODULE_SUM] = {0};  // current Temperature
	int level[MODULE_SUM] = {0};     // current level
	int minIbatt = 0;
	int i = 0;

// Step1: Get currentT
	for (i=0; i<MODULE_SUM; i++) {
		currentT[i] = get_hw_temperature(i);

		if (currentT[i] == -100) {
			printk ("SMART_CHARGER : ============= Smart Charging Stop... =============");
			return 0;
		}
	}

// Step2: currentT => level
	level[THERMAL_MODULE] = Thermal_TempToLevel(currentT[THERMAL_MODULE]);
	level[BATTERY_MODULE] = Battery_TempToLevel(currentT[BATTERY_MODULE]);
	level[CHARGER_MODULE] = Charger_TempToLevel(currentT[CHARGER_MODULE]);

// Step3: level => min Ibattery
	minIbatt = LevelToIbatt(level);
	printk ("SMART_CHARGER : minIbatt=%d\n", minIbatt);

// Step4: Set Ibattery to Charging IC
	if (minIbatt != SMflag) {
		stop_charging = false;
		if (minIbatt == 1536) {
			//1536mA
			bq24196_set_ichg(0x10);
			bq24196_set_ichg_20(0x0);
			printk ("SMART_CHARGER : ****** Set Ibattery to Charging IC : %d ******\n", minIbatt);
		} else if (minIbatt == 512) {
			//512mA
			bq24196_set_ichg(0x0);
			bq24196_set_ichg_20(0x0);
			printk ("SMART_CHARGER : ****** Set Ibattery to Charging IC : %d ******\n", minIbatt);
		} else if (minIbatt == 128) {
			//128mA
			bq24196_set_ichg(0x2);
			bq24196_set_ichg_20(0x1);
			printk ("SMART_CHARGER : ****** Set Ibattery to Charging IC : %d ******\n", minIbatt);
		}
	}
	...
	printk ("SMART_CHARGER : bq24196_get_chg_config()=%d, bq24196_get_ichg()=%d, stop_charging=%d \n",
							bq24196_get_chg_config(), bq24196_get_ichg(), stop_charging);

	SMflag = minIbatt;
	for (i=0; i<MODULE_SUM; i++) {
		Module_Table[i].oldTemp = currentT[i];
	}
}
#endif
...
///////////////////////////////////////////////////////////////////////////////////////////
//// Create File For TP : Chargeing_state
///////////////////////////////////////////////////////////////////////////////////////////
static ssize_t show_Chargeing_state(struct device *dev,struct device_attribute *attr, char *buf)
{
	if (stop_charging == false) {
		stop_charging = true; // stop charging
	} else if (stop_charging == true) {
		stop_charging = false; // start charging
	}

	return sprintf(buf, "%d\n", !stop_charging);
}

static ssize_t store_Chargeing_state(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[EM] Not Support Write Function\n");
	return size;
}

static DEVICE_ATTR(Charger_state, 0444, show_Chargeing_state, store_Chargeing_state);
...
//    wake_lock_init(&battery_suspend_lock, WAKE_LOCK_SUSPEND, "battery wakelock");
    ...
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_Charger_state);
