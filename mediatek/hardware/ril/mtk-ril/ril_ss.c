...
static void requestSendUSSD(void * data, size_t datalen, RIL_Token t)
{
    ...
    if (strlen(p_ussdRequest)/4 > MAX_RIL_USSD_NUMBER_LENGTH) {
        LOGE("USSD stringlen = %d, max = %d", strlen(p_ussdRequest)/4, MAX_RIL_USSD_NUMBER_LENGTH);

        goto error;
    }
    //END mtk08470 [20130109][ALPS00436983]
    asprintf(&cmd, "AT+ECUSD=2,1,\"%s\",72", p_ussdRequest); /* <dcs> = 0x48 */
...
static void onUssd(char *s, RILId rid)
{
    ...
    if (at_tok_hasmore(&line)) {
        ...
        p_data[1] = p_str;
        p_data[2] = (char *) GsmCbsDcsStringp[checkCbsDcs(dcs)];
    }
