...
void updateNitzOperInfo(RILId rid) {
    ...
    LOGD("[RIL%d] NITZ Operator Name : %s %s %s", (getMappingSIMByCurrentMode(rid) == GEMINI_SIM_1) ? 1 : 2,
...
void requestGetPacketSwitchBearer(RILId rid)
{
    ...
    LOGD("[RIL%d] AT+EPSB return %d", (rid==MTK_RIL_SOCKET_1)?1:2, ret);
    at_response_free(p_response);

    return;

error:
    LOGE("[RIL%d] AT+EPSB return ERROR", (rid==MTK_RIL_SOCKET_1)?1:2);
    at_response_free(p_response);
}
...
void requestSN(RILId rid) {
    ...
    LOGD("[RIL%d] Get serial number: %s", (rid==MTK_RIL_SOCKET_1)?1:2, sv);
...
void onRestrictedStateChanged(const RILId rid)
{
    ...
    if (RIL_is3GSwitched()) {
#ifdef MTK_GEMINI
        if ((rid == MTK_RIL_SOCKET_1 && (sim_inserted_status & SIM2_INSERTED))
            || (rid == MTK_RIL_SOCKET_2 && (sim_inserted_status & SIM1_INSERTED)))
#endif
        {
...
int isSimInserted(RILId rid) {
    ...
    if (RIL_is3GSwitched()) {
        if (rid == MTK_RIL_SOCKET_1) {
            if ((sim_inserted_status & SIM2_INSERTED) == SIM2_INSERTED) {
                simInserted = 1;
            }
        } else {
            if ((sim_inserted_status & SIM1_INSERTED) == SIM1_INSERTED) {
                simInserted = 1;
            }
        }
    } else {
