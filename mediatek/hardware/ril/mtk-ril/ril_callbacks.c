...
static void initializeCallback(void *param)
{
	...
	if (1 == nRadioState)
        ...
        /* Change the GPRS preference to data prefer
        /  0 - DATA Prefer
        /  1 - CALL Prefer                           */
        at_send_command("AT+EGTP=0", NULL, getDefaultChannelCtx(rid));
        /* Don't monitor PCH of other SIM when this SIM is in data transfer
        /  When one SIM is in data ransfer, to monitor PCH of the other SIM
        /  would improve the MT call successful rate, this would decrease
        /  the throughput of data trasfer.
        /  0 - Don't monitor
        /  1 - Monior                                                       */
	...
	while (s_main_loop) {
        ...
        if (continueInit) {
            ...
            if (isDualTalkMode()) {
                ...
                property_get(PROPERTY_RIL_CURRENT_SHARE_MODEM, property_value, NULL);
        		curr_share_modem = atoi(property_value);
            }
...
int isFlightModePowerOffModem() {
    /*int result = 0;
    char sim[2] = {0};
    property_get(PROPERTY_FLIGHT_MODE_POWER_OFF_MD, sim, 0);
    if (strcmp(sim, "1") == 0)
        result = 1;
...
int getFirstModem() {
	...
	char md[2] = {0};
	property_get(PROPERTY_FIRST_MD, md, 0);
	if (strlen(md) > 0)
        result = atoi(md);
    else
        result = FIRST_MODEM_NOT_SPECIFIED;
