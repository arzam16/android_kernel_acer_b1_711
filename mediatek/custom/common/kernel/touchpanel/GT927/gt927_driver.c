...
#if GTP_ESD_PROTECT
#define TPD_ESD_CHECK_CIRCLE        100
...
#ifdef TPD_CHARGER_DET
extern bool upmu_is_chr_det(void);
#define TPD_FEQ_HOP_REG 0x8030
#define TPD_BIT_FREQ                0xC6E8
#define TPD_FREQ_FACTOR             0xC6EB

static u8 charger_plug = 0;
static u8 charger_state = 0;
#endif
...
static struct proc_dir_entry *gt927_PanelType_proc = NULL;
...
static int gt927_paneltype_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    count = sprintf(page, "%i\n", g_paneltype);
    return count;
}
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    else
    {
        ...
        g_paneltype = rd_cfg_buf[GTP_ADDR_LENGTH];
   }
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    */
    ...
    // Create proc file system for panel type
    gt927_PanelType_proc = create_proc_entry(GT927_PANELTYPE_PROC_FILE, 0444, NULL);

    if (gt927_PanelType_proc == NULL)
    {
        GTP_ERROR("create_proc_entry %s failed\n", GT927_PANELTYPE_PROC_FILE);
    }
    else
    {
        gt927_PanelType_proc->read_proc = gt927_paneltype_read_proc;
...
static void gtp_esd_check_func(struct work_struct *work)
{
    ...
    charger_state = upmu_is_chr_det(); // upmu_get_PCHR_CHRDET
    if (charger_plug & !charger_state)
    {
        GTP_INFO("Unplug charger detected~!");
        ret = i2c_read_bytes(i2c_client_point, TPD_BIT_FREQ, &buf, 1);
        GTP_DEBUG("TPD_BIT_FREQ %d", buf);
        bit_freq = buf << 8;
        ret = i2c_read_bytes(i2c_client_point, (TPD_BIT_FREQ + 1), &buf, 1);
        GTP_DEBUG("TPD_BIT_FREQ+1 %d", buf);
        bit_freq += buf;
        ret = i2c_read_bytes(i2c_client_point, TPD_FREQ_FACTOR, &freq_factor, 1);
        GTP_DEBUG("TPD_FREQ_FACTOR %d", freq_factor);
        current_freq = bit_freq * freq_factor;
        GTP_DEBUG("Current_freq %d", current_freq);

        if (current_freq > 300 * 1000)
        {
            GTP_INFO("Freq > 300k ~!");
            GTP_INFO("Force reset~!");
            buf = 0;
            ret = i2c_write_bytes(i2c_client_point, TPD_FEQ_HOP_REG, &buf, 1);
            gtp_reset_guitar(i2c_client_point, 20);
            charger_plug = charger_state;
        }
    }
    ...
    do
    {
        ...
        if (touch_num)
        {
            for (i = 0; i < touch_num; i++)
            {
                ...
                id = coor_data[0];
                input_x  = coor_data[3] | coor_data[4] << 8;
                input_y  = coor_data[1] | coor_data[2] << 8;
